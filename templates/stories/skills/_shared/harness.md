...
[story.template.md](../../assets/story.template.md) — keep every `##` heading
and `<!-- phase-gate -->` comment.

## Harness contract (card skills)

Devflow owns phase transitions, locks, history, exit-script gates, and commits.
You only read context and edit `card.md` (and docs when a skill explicitly
allows).

**DO NOT:** `git commit`, `git push`, `devflow card advance`, or modify
`state.json`.
