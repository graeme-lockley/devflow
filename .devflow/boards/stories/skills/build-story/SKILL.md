...
**Philosophy:** Ship the plan **one Build Task at a time** — minimal code, passing
tests, one honest line in **Build Notes**. The harness build loop runs
`deno fmt`, `deno task ci`, and automated scenarios between rounds; you focus on
tasks and notes.
...
Harness:   deno fmt / deno task ci / scenarios (between rounds, not your job)
```
